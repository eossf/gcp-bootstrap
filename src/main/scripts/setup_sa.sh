...
if [ -z "${f}" ]; then
  ...
  echo "Verifying project parent folder..."
...
if [[ $PARENT_FOLDER_ID == "" ]]; then
  echo "Skipping grant roles on project parent folder... (parameter not passed)"
else
  echo "Applying permissions for parent folder $PARENT_FOLDER_ID..."
  # Grant roles/resourcemanager.folderViewer to the Seed Service Account on the parent folder
